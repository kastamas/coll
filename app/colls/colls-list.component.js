  ...
  .controller("CollsListCtrl", [
    ...
    function($scope, $http, $route, $cookies, $filter, $window) {
      ...
      ctrl.showMoreFilters = function() {
        ctrl.filter.more == true
          ? (ctrl.filter.more = false)
          : (ctrl.filter.more = true);
      };
      ...
      ctrl.updateCollection = function() {
        ...
        $http
          .get("/api/collocations/?" + parameters)
      ...
      ctrl.updateCounters = function() {
        ...
        if (ctrl.filter.text_id) {
          ctrl.collocationsTotalInTextQuantity =
            ctrl.quantity.totalInText[0].count;
          ctrl.collocationsFilteredQuantity = ctrl.list.length;

          ctrl.collsFilteredInPercents =
            ctrl.collocationsFilteredQuantity /
            ctrl.collocationsTotalInTextQuantity;
        }
      };
        ...
        .success(function(data, status, headers, config) {
          ...
          $scope.characteristicThreeFilter1 = function(item) {
            if (ctrl.filter.characteristic_attr1 != null) {
              return (
                item.expansion &&
                item.characteristic_id == ctrl.filter.characteristic_attr1
              );
            } else {
          ...
          $scope.characteristicThreeFilter2 = function(item) {
            return (
              item.expansion &&
              item.characteristic_id == ctrl.filter.characteristic_attr2
            );
          };
        })
        .error(function() {
      ...
      ctrl.delete = function(item_id, collocation) {
        if (confirm("Словосочетание «" + collocation + "» будет удалено.")) {
          $http
            .delete("/api/collocation/" + item_id)
